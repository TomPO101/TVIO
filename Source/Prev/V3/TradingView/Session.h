...
{
public:
	Session(std::string stockMarket, std::string stockTicker, std::string timeInterval, std::string timeFrame, unsigned int ID=-1, bool replay=false, bool recordMessages=true);
	~Session(void);
...
private:
	...
	bool RequestMoreData(unsigned int type, unsigned int amount);
