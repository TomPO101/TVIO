#pragma once
#include "Network.h"
#include "TradingCommon/TFtoTD.h"
...
#define POINT_TOO_DEEP ("point_too_deep")
...
public:
	...
	void SetMessage(const std::string& jsonStr);
	void SetType(const MessageType type) { m_Type = type; };

	void Display(void);
	
	const std::string GetMethod(void) const;
	Document& GetMessage(void) { return m_Message; };
	const std::string GetPreparedMessage(void);
	const MessageType& GetType(void) const { return m_Type; };
	MemoryPoolAllocator<CrtAllocator>& GetAllocator(void) { return m_Message.GetAllocator(); };
...
#pragma warning(suppress:4984)
	if constexpr (std::is_integral_v<ParamType>)  // check if int
		m_Message[PARAM_KEY].PushBack(Value(value), m_Message.GetAllocator());
	else
		m_Message[PARAM_KEY].PushBack(Value(value, m_Message.GetAllocator()), m_Message.GetAllocator());
}
